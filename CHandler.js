...
defineAdminSession = function(handler){
    handler.adminSession = new josm.Session("MainSession", "", []); //admin session is public..

    handler.adminSession.createModule("AdminModule", "godfather"); //admin module is private, however..
    ...
    handler.adminSession.createFunction("AdminModule", "ResetPassword", rpBody, rpInput);

    handler.adminSession.createModule("Public", ""); //'Public' session is public. Methods for everyone!
    var pfBody = "console.log(msg)";
    var pfInput = ["msg"];
    handler.adminSession.createFunction("Public", "Shout", pfBody, pfInput);

    handler.sessions.put(Number(handler.adminSession.id), handler.adminSession);
    return handler.adminSession;
};
...
CHandler.prototype.addUser = function(type, sendStream){
...
CHandler.prototype.removeUser = function(type, sendStream){
...
CHandler.prototype.sessionBroadcast = function(user, sessionID, msg){
...
CHandler.prototype.onMessage = function(sentMsg){    
    //msg.UserID;
    //msg.SessionID;
    //msg.ModuleID;
    //msg.FunctionID;
...
CHandler.prototype.respond = function(user, type, args){
...
CHandler.prototype.sessionList = function(type){
...
CHandler.prototype.getAvailableSessionTypes = function(){
    return this.loadedTypes.listKeys();    
};

CHandler.prototype.getActiveSessions = function(type){ //returns active session by specified type
...
CHandler.prototype.createSession = function(tag, password, properties){
...
CHandler.prototype.setSessionOnEvent = function(sessionType, event, value){
    var session = this.loadedTypes.get(sessionType);
    if(session == undefined){
        console.log("CHandler.setSessionOnEvent:: error finding session type: " + sessionType);
        return;
    }
    session.setOnEvent(event, value);
};

CHandler.prototype.setPersistent = function(sessionType, persistent){
    var session = this.loadedTypes.get(sessionType);
    if(session == undefined){
        console.log("CHandler.setPersistent:: error finding session type: " + sessionType);
        return;
    }
